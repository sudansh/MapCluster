...
class AtmDaoTest : KoinTest {
    ...
    fun testSaveAndFind() {
        val dao = db.atmDao()
        val entities = createAtmList("foo", 123.0, 456.0, 10)

        // Save entities
        dao.insert(entities)

        // Keep id for each one
        val ids = entities.map { it.sonectId }
    ...
    fun testFindAllBy() {
        val dao = db.atmDao()
        val entity = createAtm("1", "foo", 123.0, 456.0)
        dao.insert(entity)
        val entity2 = createAtm("2", "fooo", 1234.0, 4567.0)
        dao.insert(entity2)
